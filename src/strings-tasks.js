...
function sumOfCodes(str) {
  ...
  const newStr = str
    .split('')
    .reduce((sumChar, current) => sumChar + current.charCodeAt(), 0);
  return newStr;
}
...
function countVowels(str) {
  ...
  return str.split('').reduce((sumVowels, current) => {
    return Vowels.includes(current) ? sumVowels + 1 : sumVowels;
  }, 0);
}
...
function isPalindrome(str) {
  const newString = str.replaceAll(/\W/gi, '').toLowerCase();
  const newReverseString = newString.split('').reverse().join('');
  return newString === newReverseString;
}
...
function findLongestWord(sentence) {
  // const sort = sentence.split(' ').sort((a, b) => a.length - b.length);
  let result = '';
  sentence.split(' ').forEach((elem) => {
    if (elem.length > result.length) result = elem;
  });
...
function invertCase(str) {
  return str
    ...
    .map((elem) => {
      let newElem;
      if (elem === elem.toLowerCase()) newElem = elem.toUpperCase();
      if (elem === elem.toUpperCase()) newElem = elem.toLowerCase();
      return newElem;
    })
    .join('');
}
...
function encodeToRot13(str) {
  const firstPartM = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
  const secondPartM = 'abcdefghijklmnopqrstuvwxyz';
  const newStr = str.split('').map((elem) => {
    if (firstPartM.indexOf(elem) >= 0)
      return firstPartM.charAt((firstPartM.indexOf(elem) + 13) % 26);
    if (secondPartM.indexOf(elem) >= 0)
      return secondPartM.charAt((secondPartM.indexOf(elem) + 13) % 26);
    return elem;
  });
  return newStr.join('');
}
